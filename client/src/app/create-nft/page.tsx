...
const CreateReelPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    let id = toast.loading("Uploading your ID...");
    await delay(2000);
    e.preventDefault();
    console.log({
      video,
    ...
    toast.success("Id uploaded successfully!", { id });
  };
  ...
  return (
    <div className="min-h-screen bg-dark-3 flex items-center justify-center p-4">
      <div className="w-full max-w-lg bg-dark-2 p-8 rounded-lg shadow-lg">
        <h1 className="text-2xl font-bold mb-6 text-light-1">Create NFTS</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex w-3/4 flex-col mt-10">
                  <label className="block" htmlFor="name">
                    <input
                      ...
                      type="email"
                      placeholder="Enter your nft Link"
                    />
                  </label>
          ...
          <div className="flex items-center space-x-3">
            ...
            >
              Monetize your NFTs with Evo-ID
            </label>
          </div>
